...
vi.mock('framer-motion', () => {
  const motionPropNames = new Set(['initial','animate','exit','variants','transition','whileHover','whileTap','whileInView','whileFocus','whileDrag','layout','layoutId','onAnimationComplete','onAnimationStart','custom','inherit']);
  const fp = (p: Record<string, unknown>) => Object.fromEntries(Object.entries(p).filter(([k]) => !motionPropNames.has(k)));
  return {
    motion: {
      div: ({ children, className, onClick, onMouseDown, onMouseUp, onTouchStart, onTouchEnd, style, ...props }: React.PropsWithChildren<{ className?: string; onClick?: () => void; onMouseDown?: () => void; onMouseUp?: () => void; onTouchStart?: () => void; onTouchEnd?: () => void; style?: React.CSSProperties }>) => (
        <div className={className} onClick={onClick} onMouseDown={onMouseDown} onMouseUp={onMouseUp} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} style={style} {...fp(props)}>{children}</div>
      ),
      button: ({ children, className, onClick, disabled, ...props }: React.PropsWithChildren<{ className?: string; onClick?: () => void; disabled?: boolean }>) => (
        <button className={className} onClick={onClick} disabled={disabled} {...fp(props)}>{children}</button>
      ),
      h1: ({ children, className, ...props }: React.PropsWithChildren<{ className?: string }>) => (
        <h1 className={className} {...fp(props)}>{children}</h1>
      ),
      h2: ({ children, className, ...props }: React.PropsWithChildren<{ className?: string }>) => (
        <h2 className={className} {...fp(props)}>{children}</h2>
      ),
      p: ({ children, className, ...props }: React.PropsWithChildren<{ className?: string }>) => (
        <p className={className} {...fp(props)}>{children}</p>
      ),
      span: ({ children, className, ...props }: React.PropsWithChildren<{ className?: string }>) => (
        <span className={className} {...fp(props)}>{children}</span>
      ),
      circle: (props: React.SVGProps<SVGCircleElement>) => <circle {...fp(props)} />,
    },
