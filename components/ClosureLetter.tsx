...
const ClosureLetter: React.FC<ClosureLetterProps> = ({ isOpen, onClose, message }) => {
  ...
  // Uses requestAnimationFrame for smooth, jank-free text reveal
  useEffect(() => {
    if (stage !== 'reading' || hasBeenReadRef.current) return undefined;

    let rafId: number | null = null;
    let startTime: number | null = null;
    const charsPerSecond = 120; // Smooth reveal speed
    const initialDelay = 1200; // Delay to allow paper to unfold

    const timer = setTimeout(() => {
        const tick = (timestamp: number) => {
            if (startTime === null) startTime = timestamp;
            const elapsed = timestamp - startTime;
            const chars = Math.min(message.length, Math.floor((elapsed / 1000) * charsPerSecond));
  ...
  // Skip to end handler for impatient users
                    ...
                    {stage === 'reading' && (
                        ...
                        >
                            ...
                            <div className="relative z-10 flex flex-col h-full p-8 md:p-12 overflow-hidden">
                                ...
                                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0" onClick={handleSkipToEnd}>
                                    <p className="text-xl md:text-2xl leading-relaxed whitespace-pre-wrap text-[#2c1810] font-handwriting">
                                        {/* Visible portion */}
                                        <span className="letter-text-visible">{message.slice(0, visibleChars)}</span>
                                        {/* Cursor */}
                                        {visibleChars < message.length && (
                                            <span className="inline-block w-[2px] h-[1.1em] bg-gradient-to-b from-[#2c1810]/60 to-[#2c1810]/20 ml-[1px] align-middle animate-letter-cursor" />
                                        )}
                                        {/* Hidden remainder for layout stability */}
                                        <span className="invisible">{message.slice(visibleChars)}</span>
                                    </p>
                                </div>
