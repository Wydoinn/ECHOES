...
const AlchemicalVortex: React.FC<AlchemicalVortexProps> = ({ phase, reducedMotion = false }) => {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      const currentPhase = phaseRef.current;
      const ease = currentPhase === 'coalesce' ? 0.08 : 0.04;

      // Pre-compute shared time values
      const timeTwinkle = time * 0.005;
      ...
      for (let i = 0; i < count; i++) {
        ...
        const twinkle = Math.sin(timeTwinkle + p.alphaOffset);
        const alpha = 0.4 + twinkle * 0.3;

        if (alpha < 0.05) continue;

        const absZ = p.z < 0 ? -p.z : p.z;
        const distAlpha = absZ < 1000 ? 1 : 1000 / (absZ + 1);

        const combinedAlpha = alpha * distAlpha;
        if (combinedAlpha < 0.02) continue;
        ...
        // Add soft glow ring for larger particles only
        if (radius >= 2) {
            ctx.fillStyle = `rgba(${r},${g},${b},${combinedAlpha * 0.3})`;
            ctx.beginPath();
