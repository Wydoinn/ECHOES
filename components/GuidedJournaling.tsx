...
const GuidedJournaling: React.FC<GuidedJournalingProps> = ({
  ...
  const goToStep = (stepIndex: number) => {
    if (stepIndex <= completedSteps.length) {
      ...
      onPromptChange(steps[stepIndex].prompt);
    }
  };
  ...
  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      ...
      onPromptChange(steps[nextStep].prompt);
    }
  };
        ...
        >
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] text-white/40">
              {currentWordCount} / {currentStepData.minWords}+ words
            </span>
          </div>
          ...
          <div className="h-1 bg-white/10 rounded-full overflow-hidden">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{
                width: `${Math.min(100, (currentWordCount / currentStepData.minWords) * 100)}%`
              }}
